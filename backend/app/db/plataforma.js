...
export async function existePlataformaEnPais(pais, plataformaIdExcluida = null) {
    let query = "SELECT COUNT(*) FROM PLATAFORMA WHERE LOWER(TRIM(PAIS)) = LOWER(TRIM($1))";
    const params = [pais];
    if (plataformaIdExcluida) {
        query += " AND Plataforma_Id != $2"
        params.push(plataformaIdExcluida)
    }
